...
!!! note "Optional"
    ...
    You need to set up the backend separately—either by updating the endpoint URL to point to an existing backend, or by deploying the [sample backend](../../../../assets/attachments/pizzashack.war) if you need to try out the sample PizzaShack API.